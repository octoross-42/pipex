...
/*   Updated: 2024/02/02 18:07:43 by octoross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_get_cmd(char *cmd, char **paths)
{
	...
	return (printf(ERR_CMD, cmd), NULL);
}
...
char	**ft_get_path(char **envp)
{
	...
	while (envp[i])
	{
		if (strncmp(envp[i], "PATH=", 5) == 0) // TODO ft_strncmp
		{
			...
			if (!path)
				return (printf(ERR_MALLOC), NULL); // TODO ft_printf
			paths = ft_split(path, ':');
			free(path);
			if (!paths)
...
int	main(int argc, char **argv, char **envp)
{
	...
	while (i < pipex.nbr_cmds - 1)
		ft_do_pipe(argv[i + 2 + pipex.here_doc], pipex.path, envp);
	if (pipex.here_doc)
